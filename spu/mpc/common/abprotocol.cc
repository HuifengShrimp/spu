...
//lj
#define _LogRegAP(A, B, C, M, N, K) ctx->caller()->call("logreg_ap", A, B, C, M, N, K)
#define _LogRegAA(A, B, C, M, N, K) ctx->caller()->call("logreg_aa", A, B, C, M, N, K)
...
//lj
ArrayRef ABProtLogRegPP::proc(KernelEvalContext* ctx, const ArrayRef& A, const ArrayRef& B,
                              const ArrayRef& C, size_t M, size_t N,
                               size_t K) const {
  SPU_TRACE_KERNEL(ctx, A, B, C);
  return _LogRegAP(A, B, C, M, N, K);
}

ArrayRef ABProtLogRegSS::proc(KernelEvalContext* ctx, const ArrayRef& A, const ArrayRef& B,
                              const ArrayRef& C, size_t M, size_t N,
                               size_t K) const {
  SPU_TRACE_KERNEL(ctx, A, B, C);
  return _LogRegAA(A, B, C, M, N, K);
}
