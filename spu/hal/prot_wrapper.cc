...
#define MAP_MMUL_OP(NAME)                                                  \
  ...
  //lj-todo
#define MAP_LOGREG_OP(NAME)                                                                 \
  Value _##NAME(HalContext* ctx, const Value& x, const Value& w, const Value& y) {          \
    SPU_TRACE_HAL(ctx, x, w, y);                                                            \
    ctx->prot()->setTracingDepth(ctx->getTracingDepth());                                   \
    auto [m, n, k] = deduceMmulArgs(x.shape(), w.shape());                                  \
    auto ret =                                                                              \
        mpc::NAME(ctx->prot(), flattenValue(x), flattenValue(w), flattenValue(y), m, n, k); \
    return unflattenValue(ret, {m, n});                                                     \
  }
...
//lj
MAP_LOGREG_OP(logreg_pp)
MAP_LOGREG_OP(logreg_ss)
